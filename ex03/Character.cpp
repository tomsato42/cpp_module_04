...
{
    init_moves();
    std::cout << "\033[34mCharacter default constructor called. Name set to \033[1m" << name_ << "\033[0;34m.\033[0m"
              << std::endl;
}
Character::Character(std::string const &name) : name_(name)
{
    init_moves();
    std::cout << "\033[34mCharacter constructor called. Name set to \033[1m" << name_ << "\033[0;34m.\033[0m"
              << std::endl;
}
...
Character::Character(const Character &other)
{
    ...
    std::cout << "\033[34mCharacter copy constructor called. Name set to \033[1m" << name_ << "\033[0;34m.\033[0m"
              << std::endl;
}
...
Character::equip(AMateria *m)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        if (!moves[i])
        {
            std::cout << "\033[34mEquipped \033[1m" << m->getType() << "\033[0;34m to slot \033[1m" << i
                      << "\033[0;34m.\033[0m" << std::endl;
            moves[i] = m;
            return;
        }
    }
...
Character::unequip(int idx)
{
    if (idx < 0 || idx >= 4)
    {
        std::cout << "\033[34mInvalid index: \033[1m" << idx << "\033[0;34m. Must be between 0 and 3.\033[0m"
                  << std::endl;
        return;
    }
    if (!moves[idx])
...
Character::use(int idx, ICharacter &target)
{
    ...
    std::cout << "\033[34m\033[1m" << name_ << "\033[0;34m uses \033[1m" << moves[idx]->getType()
              << "\033[0;34m on \033[1m" << target.getName() << "\033[0;34m.\033[0m" << std::endl;
